...
//===========Csv Loaders===========
const { loadAllCSVs, concepts,findTM1Match, findTM2Match,applySynonyms,stripHumours} = require("./models/cvs");
loadAllCSVs();
...
//=================send mail================
const { generateUsername, generateRandomPassword, sendCredentials } = require("./models/credentials");
...
function cleanQuery(text) {
  return text.replace(/classified under/gi, "").replace(/vali humour/gi, "").replace(/[^a-zA-Z0-9\s]/g, "").trim();
}
...
app.post("/user/change-password", async (req, res) => {
  ...
  connection.query(sql, [userId], async (err, results) => {
    ...
    connection.query("UPDATE users SET password_hash = ? WHERE id = ?", [hashed, userId], (err2) => {
      if (err2) {
...
app.post("/user/forgot-password", async (req, res) => {
  ...
  connection.query(sql, [email], async (err, results) => {
    ...
    connection.query(updateSQL, [hashedPassword, user.id], async (updateErr) => {
      if (updateErr) {
...
app.post("/user/register", async (req, res) => {
  ...
  if (!firstname || !lastname || !email) {
    return res.status(400).send("Missing required fields (firstname, lastname, email required)");
  }
  ...
  try {
    ...
    connection.query(checkEmailSQL, [email], async (err, results) => {
      ...
      connection.query(insertSQL, [firstname, lastname, username, email, hashedPassword, dob || null, phone || null], async (err) => {
        if (err) {
        ...
        try {
          await sendCredentials(email, firstname, lastname, username, plainPassword);
...
app.post("/user/send-otp", async (req, res) => {
  ...
  if (record && now < record.expiresAt) {
    ...
    return res.status(400).json({ error: `Wait ${remaining} seconds before requesting new OTP` });
  }
...
app.post("/user/verify-otp", (req, res) => {
  ...
  if (Date.now() > record.expiresAt) return res.status(400).json({ error: "OTP expired. Request again." });
...
app.get("/lookup", (req, res) => {
  try {
    ...
    let matches = concepts
      .filter((item) => Object.values(item).filter(Boolean).some((val) => val.toString().toLowerCase().includes(q)))
      .map((item) => {
        const formatted = formatDefinition(item.long_definition);
        return {
          ...item,
          full_long_definition: item.long_definition,
          long_definition: item.long_definition ? item.long_definition.split(" ").slice(0, 10).join(" ") + " ..." : item.long_definition,
          formatted_definition: formatted,
          icdPublicUri: item.NUMC_CODE ? `https://icd.who.int/browse/2025-01/mms/en#${item.NUMC_CODE}` : null,
        };
      });
    ...
    if (sortBy === "code") {
      ...
      matches = matches.sort((a, b) => (a.term || "").localeCompare(b.term || ""));
    }
    if (matches.length === 1) {
...
app.get("/map/:code", async (req, res) => {  
  try {
    ...
    if (!match) {
      return res.status(404).json({ message: "Code not found in NAMASTE dataset" });
    }
    ...
    if (tm1Match) {
      ...
      response.icdDetailsTM1 = { 
        message: "TM1 dataset entry missing. Please check the official TM1 site.",
    ...
    if (tm2Match) {
      ...
      response.icdDetailsTM2 = { 
        message: "TM2 dataset entry missing. Please check the official TM2 site.",
        redirect: "https://icd.who.int/browse/tm/en"
      };
    }
...
app.get("/fhir/ValueSet/$lookup", ensureLoggedIn,async (req, res) => {
  try {
    ...
    const match = concepts.find(c => c.code?.toUpperCase() === code.toUpperCase());
    if (!match) {
      return res.json({
        resourceType: "ValueSet",
        status: "active",
        expansion: { timestamp: new Date().toISOString(), total: 0, contains: [] }
      });
    }
...
async function doTranslation(codeOrMatch) {
  const match = typeof codeOrMatch === "string" ? concepts.find(row => row.code?.toUpperCase() === codeOrMatch.toUpperCase()): codeOrMatch;
  if (!match) return { mappings: [], match: null };

  let mappings = [];
  mappings.push({system: "NAMASTE",code: match.code,display: cleanHTML(match.term)});
  ...
  try {
    ...
    if (ICD_API_KEY) {
      ...
      const baseQuery = match.short_definition || match.term;
      const queries = [cleanQuery(baseQuery),cleanQuery(match.term),cleanQuery(baseQuery.split(" ").slice(0, 2).join(" ")),];
      ...
      if (topEntity) {
        mappings.push({system: "ICD11-Bio",code: topEntity.theCode || topEntity.code || "",display: cleanHTML(topEntity.title || topEntity.term || "-")});
      }
    }
  } catch (err) {
  ...
  let tm1Match = findTM1Match(queryTerm);
  if (!tm1Match && queryTerm.includes(" ")) tm1Match = findTM1Match(queryTerm.split(" ").slice(0, 2).join(" "));
  if (tm1Match) mappings.push({ system: "ICD11-TM1", code: tm1Match.tm_code, display: cleanHTML(tm1Match.disorder) });

  let tm2Match = findTM2Match(queryTerm);
  if (!tm2Match && queryTerm.includes(" ")) tm2Match = findTM2Match(queryTerm.split(" ").slice(0, 2).join(" "));
  if (tm2Match) mappings.push({ system: "ICD11-TM2", code: tm2Match.tm_code, display: cleanHTML(tm2Match.disorder) });
...
app.get("/fhir/ConceptMap/$translate", ensureLoggedIn, async (req, res) => {
  try {
    ...
    const uniqueMappings = Array.from(new Map(mappings.map(m => [m.system + m.code, m])).values());

    res.json({
      resourceType: "Parameters",
      parameter: uniqueMappings.map(m => ({
        name: "match",
        valueCoding: {system: m.system,code: m.code,display: cleanHTML(m.display)}
      }))
    });
...
app.post("/fhir/ConceptMap/$translate", ensureLoggedIn, async (req, res) => {
  try {
    ...
    const match = concepts.find(row => row.code?.toUpperCase() === code.toUpperCase());
    if (!match) {
      return res.render("translate-result.ejs", { mappings: [], user: req.session?.user || null });
    }
    ...
    if (ICD_API_KEY) {
      ...
      if (topEntity) {
        mappings.push({
          ...
          display: cleanHTML(topEntity.title) || cleanHTML(topEntity.term) || cleanHTML(topEntity.name) || "-"
        });
      }
    }
    ...
    if (tm1Match) {
      mappings.push({ system: "ICD11-TM1", code: tm1Match.tm_code, display: tm1Match.disorder });
    }
    ...
    if (tm2Match) {
      mappings.push({ system: "ICD11-TM2", code: tm2Match.tm_code, display: tm2Match.disorder });
    }
    mappings.forEach(m => {
      ...
      connection.query(sql, ["NAMASTE", match.code, match.term, m.system, m.code, m.display]);
    });
...
app.post("/fhir/Encounter/$upload", ensureLoggedIn, async (req, res) => {
      ...
      if (result?.mappings) {
        reason.coding.push(
          ...result.mappings
            ...
            .map(m => ({ system: m.system, code: m.code, display: m.display }))
        );
      }
    }
  }
  ...
  connection.query(sqlCheck, [patientId], (err, rows) => {
    ...
    if (rows[0].count > 0) {
      ...
      return res
        .status(400)
        .json({ error: `Patient ID '${patientId}' already exists. Please use a new ID.` });
    }
    ...
    connection.query(sqlInsert, [patientId, JSON.stringify(bundle)], (err2) => {
      ...
      res.json({ message: `FHIR bundle stored successfully of pateint with ${patientId}`});
    });
  });
});
  ...
  connection.query(sql, [patientId], (err, results) => {
    ...
    res.json({
      ...
      entry: results.map(r => ({
        ...
        resource: typeof r.bundle === 'string' ? JSON.parse(r.bundle) : r.bundle,
